<?php
	...
	//access the information and autofill it in the box
	$fetchSql = "SELECT * FROM $tblNameOne WHERE productId = '$productId'";
	$fetchQuery = mysqli_query($connection,$fetchSql);
	$fetchResults = mysqli_num_rows($fetchQuery);
	if($fetchResults>0){
		while($fetchedProduct = mysqli_fetch_assoc($fetchQuery)){
				$productName = $fetchedProduct['productName'];
				$productPrice =  $fetchedProduct['productPrice'];
				$productAvailability = $fetchedProduct['productAvailability'];
				$productImage = $fetchedProduct['productImage'];
		}
	}
	//finished auto filling
			...
			if(!empty($fileOneName)){
				...
				if(in_array($fileOneActualExt,$allowedExtensions)){
					...
					if(move_uploaded_file($fileOneTmpName,$fileOneDestination)){
						...
						$sql = "UPDATE $tblNameOne SET productId='$productId', productName='$productName',productPrice='$productPrice',productImage='$imageOne',productAvailability='$productAvailability' WHERE productId='$productId';";
						$query = mysqli_query($connection,$sql);
						//$results = mysqli_num_rows($query);
						if($query){
							header( "refresh:1;url=http://localhost//best//adminPortal.php" );
							echo("product updated successfuly redirecting you<br>");
						}
						else{
							//failed to upload
			...
			<div class="employee-content container d-flex justify-content-center">
				<div class="row d-flex justify-content-center">
					<div class="col-12 col-sm-12 col-md-12 col-lg-4">
						<div class="card d-flex h-100">
							<div class="card-body text-center">
								...
								<h4 class="text-center"><?php echo($productName) ?></h4>
					...
					<form enctype="multipart/form-data" style="background-image: linear-gradient(to left, #10A3D4 , #0049d5 , #004ee5 , #10A3D4);" class="col-12 col-sm-12 col-md-4 py-5 d-flex flex-column" action="<?php echo($_SERVER['PHP_SELF'])?>" method="POST">
						<h4 class="text-white text-center">Your are now editing the product on your left</h4>
						...
						<button type="submit" class="btn btn-md mt-3 text-white py-3 font-weight-bold" style="background-color:black" name="updateBtn">
							Update
						</button>
					</form>
				</div>
